...
class ActorSystemProvider @Inject()(environment: Environment, configuration: Configuration)
  extends javax.inject.Provider[ActorSystem] {
  override def get(): ActorSystem = ActorSystem("PlaySystem")
}

class MaterializerProvider @Inject()(implicit system: ActorSystem)
  extends javax.inject.Provider[Materializer] {
